    ...
    <div class="container-fluid px-3 px-md-5 py-5 mx-auto" style="max-width: 1400px;">
        ...
        <form action="treatmentUpdateProduct.php?id=<?= $product['id'] ?>" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group my-2">
               ...
               <input type="text" name="name" id="nom" class="form-control my-2" value="<?= $product['name'] ?>">
            </div>
            <div class="form-group my-2">
                <label for="description">Déscription: </label>
                <textarea name="description" id="description" class="form-control my-2"><?= $product['description'] ?></textarea>
            </div>
            <div class="form-group my-2">
                <label for="prix">Prix: </label>
                <input type="number" name="prix" id="prix" step="0.01" class="form-control my-2" value="<?= $product['prix'] ?>">
            </div>
            <div class="form-group my-2">
            ...
            <div class="form-group my-2">
                <input type="submit" value="Modifier" class="btn btn-sm btn-outline-warning my-2">
            </div>
        </form>
        <h2>Gestion des images</h2>
        <a href="addImg.php?id=<?= $product['id'] ?>" class="btn btn-sm btn-outline-primary my-2">Ajouter une image</a>
                ...
                <?php else: ?>
                    <?php foreach($images as $image) : ?>
                        <tr class="text-center">
                            ...
                            <td>
                                <a href="updateProduct.php?id=<?= $product['id'] ?>&delete=<?= $image['id'] ?>" class="btn btn-sm btn-outline-danger">Supprimer</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
